...
class ProtectBuff(engine: Engine, audioService: AudioService) : Buff(engine, audioService) {
  private var amount = 3
  private val reduceDamage = 0.5f

  override fun onEvent(event: GameEvent) {
    if (event is CombatDamageEvent && target == event.damageEmitterComponent.target) {
